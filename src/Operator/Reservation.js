		...
		try {
			...
			const reservationPromises = querySnapshot.docs.map(async (reservationDoc) => {
				...
				return {
					...
					date: new Date(reservationDoc.data().timestamp.seconds * 1000).toLocaleDateString(
						"en-US"
					),
				};
			});
			...
			try {
				...
				if (userEmail) {
					const notificationData = {
						...
						title: "Reservation",
						message: `Your reservation at ${user.managementName} on ${floorTitle} ${
							slotId + 1
						} is accepted`,
				...
				>
					...
					{showAccepted && (
						<div>
							...
							{historyLog
								.filter((logEntry) => logEntry.status === "Accepted" && filterByDate(logEntry))
								.map((logEntry, index) => (
									<div className="alert alert-success mt-2" key={index}>
										<strong>Accepted:</strong> {logEntry.name} requested a reservation on{" "}
										{logEntry.slotId}. Plate Number: {logEntry.plateNumber}, Slot: {logEntry.slotId}
									</div>
								))}
						</div>
					)}
					{showDeclined && (
						<div>
							...
							{historyLog
								.filter((logEntry) => logEntry.status === "Declined" && filterByDate(logEntry))
								.map((logEntry, index) => (
									<div className="alert alert-danger mt-2" key={index}>
										<strong>Declined:</strong> {logEntry.name} requested a reservation on{" "}
										{logEntry.slotId}. Plate Number: {logEntry.plateNumber}, Slot: {logEntry.slotId}
									</div>
								))}
						</div>
		...
		return (
            ...
            /> */}
						{request?.location && user.coordinates && (
							<MapComponent origin={request.location} destination={user.coordinates} />
						)}
					</Modal.Body>
					<Modal.Footer>
